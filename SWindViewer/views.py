...
class ChartData(APIView):
    def get(self, request, format=None):
        try:
            ...
            if mag_response.status_code != 200 or plasma_response.status_code != 200:
            ...
            graph_data = prepare_graph_data(mag_data, plasma_data, cl_24hr, cl_tnow, timestamps_24h)
            return Response(graph_data)
        except requests.RequestException as e:
            return Response({'error': 'Failed to connect to API'}, status=500)

def prepare_graph_data(mag_data: list, plasma_data: list, start_time: datetime, end_time: datetime, timestamps_24h: pd.DatetimeIndex) -> dict:
    graph_data = {
        "labelsplasma": [],
        "labelsmag": [],
        "chartspeed": [],  
